...
| Sleep sessions & stages | Yes |
| Daily activity (steps, calories, floors, distance, active minutes) | Yes |
| Resting heart rate | Yes |
| Heart rate variability (HRV / RMSSD) | Yes |
| SpO2 (blood oxygen saturation) | Yes |
| Breathing rate | Yes |
| Body weight & BMI | Yes |
| Body fat percentage | Yes |
| VO2 max (Cardio Score) | Yes |
| Heart rate (intraday) | Requires Fitbit Personal app type — not available with Server app type |

<Info>
Open Wearables syncs workouts, sleep, daily activity, and all available health metrics via polling. Intraday heart rate timeseries requires a Fitbit **Personal** app type — the default **Server** app type only exposes resting heart rate from the daily summary.
</Info>
...
FITBIT_DEFAULT_SCOPE="activity heartrate sleep profile oxygen_saturation respiratory_rate cardio_fitness weight"
```
    ...
    **Configuration details:**
    ...
    | `FITBIT_DEFAULT_SCOPE` | Space-separated list of OAuth scopes to request. Expanded to include `oxygen_saturation`, `respiratory_rate`, `cardio_fitness`, and `weight` for 247 data sync. **Existing connected users must re-authorize to grant new scopes.** | `activity heartrate sleep profile oxygen_saturation respiratory_rate cardio_fitness weight` |
  ...
  <Step title="Sync data">
    ...
    ```bash
    # Sync workouts/activities only
    curl -X POST "http://localhost:8000/api/v1/providers/fitbit/users/{user_id}/sync?data_type=workouts" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"

    # Sync 247 health data (sleep, activity, HRV, SpO2, body composition, VO2 max)
    curl -X POST "http://localhost:8000/api/v1/providers/fitbit/users/{user_id}/sync?data_type=247" \
      -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
